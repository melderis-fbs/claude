import {
  ...
  getMockClientesNuevosData,
  getMockRecoleccionData,
} from './mockData.js';
...
export async function getClientesNuevosData() {
  if (MOCK_MODE) return getMockClientesNuevosData();
  try { return await fetchTab('clientes'); }
  catch (err) { console.error('getClientesNuevosData:', err.message); return getMockClientesNuevosData(); }
}

export async function getRecoleccionData() {
  if (MOCK_MODE) return getMockRecoleccionData();
  try { return await fetchTab('recoleccion'); }
  catch (err) { console.error('getRecoleccionData:', err.message); return getMockRecoleccionData(); }
}
...
export async function getComentariosData() {
  if (MOCK_MODE) return {};
  try {
    const res = await fetch(`${process.env.APPS_SCRIPT_URL}?tab=comentarios`, { cache: 'no-store' });
    const json = await res.json();
    return json.data || {};
  } catch (err) {
    console.error('getComentariosData:', err.message);
    return {};
  }
}

export async function postComentario(tipo, nombre, texto) {
  if (MOCK_MODE) return { ok: true };
  try {
    const res = await fetch(process.env.APPS_SCRIPT_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'comentario', tipo, nombre, texto }),
    });
    return await res.json();
  } catch (err) {
    console.error('postComentario:', err.message);
    return { ok: false };
  }
}
