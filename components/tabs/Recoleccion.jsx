...
function PagoDot({ estado }) {
  ...
  const map = { 'Cobrado': 'bg-pos', 'Pendiente': 'bg-gold', 'Vencido': 'bg-neg' };
  ...
  return (
    <div className="p-4 bg-page border-t border-cream space-y-4" onClick={e => e.stopPropagation()}>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
        ...
        <div>
          <p className="text-ink-3 mb-0.5">Cuotas pagas</p>
          <p className="text-ink-2 font-medium">{cliente.cuotasPagas ?? '—'} / {cliente.cuotas ?? '—'}</p>
        </div>
      </div>
...
export default function Recoleccion({ data = [] }) {
  ...
  const monthCards = useMemo(() => {
    ...
    data.forEach(c => {
      (c.pagos || []).forEach(p => {
        ...
        if (!mesKey) return;
        if (!map[mesKey]) map[mesKey] = { mes: mesKey, aCobrar: 0, cobrado: 0, pendiente: 0, vencido: 0 };
        const r = map[mesKey];
        r.aCobrar += p.monto || 0;
        if (p.estado === 'Cobrado') r.cobrado += p.monto || 0;
        else if (p.estado === 'Vencido') r.vencido += p.monto || 0;
        else r.pendiente += p.monto || 0;
      });
    });
  ...
  const stats = useMemo(() => {
    ...
    data.forEach(c => {
      totalAdeudado += c.montoAdeudado || 0;
      const tieneVencido = (c.pagos || []).some(p => p && p.estado === 'Vencido');
      if (tieneVencido) enMora++;
      (c.pagos || []).forEach(p => {
        if (!p) return;
        if (p.estado === 'Cobrado') totalCobrado += p.monto || 0;
        else if (p.estado === 'Pendiente' || p.estado === 'Vencido') pendiente += p.monto || 0;
      });
    });

    return { totalClientes, totalAdeudado, totalCobrado, pendiente, enMora };
  }, [data]);
  ...
  }, [data, search, filterFuente, filterEstado]);
  ...
  return (
    <div className="space-y-4">
      ...
      {monthCards.length > 0 && (
        <div>
          <h2 className="text-xs font-semibold tracking-widest uppercase text-ink-3 mb-3">Proyección mensual</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {monthCards.map(mc => {
              const pct = mc.aCobrar > 0 ? Math.round((mc.cobrado / mc.aCobrar) * 100) : 0;
              return (
                <div key={mc.mes} className="bg-white rounded-xl border border-cream shadow-sm p-3">
                  <p className="text-xs font-semibold text-ink-3 mb-2">{mesLabel(mc.mes)}</p>
                  <div className="space-y-1.5 text-xs">
                    <div className="flex justify-between">
                      <span className="text-ink-3">A cobrar</span>
                      <span className="font-medium text-ink-2">{usd(mc.aCobrar)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-pos">Cobrado</span>
                      <span className="font-semibold text-pos">{usd(mc.cobrado)}</span>
                    </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-ink-2">{usd(mc.pendiente)}</span>
                    </div>
                    {mc.vencido > 0 && (
                      <div className="flex justify-between">
                        <span className="text-neg">Vencido</span>
                        <span className="font-medium text-neg">{usd(mc.vencido)}</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
        {[
          ...
          { label: 'Total adeudado',  val: usd(stats.totalAdeudado), cls: 'text-ink-1' },
          { label: 'Total cobrado',   val: usd(stats.totalCobrado),  cls: 'text-pos' },
          ...
          <table className="w-full min-w-max text-sm">
            <thead>
              <tr className="border-b border-cream bg-page">
                {['Cliente', 'Programa', 'Ingreso', 'Monto total', 'Adeudado', 'Cuotas', '% Cobrado', ''].map(h => (
              ...
              {filtered.map((cliente, i) => {
                ...
                return [
                  ...
                  >
                    ...
                    <td className="py-3 px-3"><ProgramaBadge programa={cliente.programa} /></td>
                    ...
                    <td className="py-3 px-3 whitespace-nowrap">
                      <span className={clsx(
                        ...
                        (cliente.montoAdeudado || 0) > 0 ? 'text-neg' : 'text-pos'
                      )}>
                        {usd(cliente.montoAdeudado || 0)}
                      </span>
                    </td>
                    <td className="py-3 px-3">
