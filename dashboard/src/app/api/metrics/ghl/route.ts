...
export async function GET(request: Request) {
  ...
  try {
    ...
    const snapshot = await getLatestSnapshot("ghl");
    if (snapshot) return NextResponse.json({ source: "cache", data: snapshot.data, recordedAt: snapshot.recorded_at });
    const data = await fetchGHLData();
    return NextResponse.json({ source: "live", data, fetchedAt: new Date().toISOString() });
  } catch (err) {
    return NextResponse.json({ error: "Failed to fetch GHL metrics", details: err instanceof Error ? err.message : "Unknown error" }, { status: 500 });
  }
}
